import React, { Component } from 'react';
import Footer from './../Footer/Footer';
import './Landing.css';

export default class Landing extends Component {
    render() {
        return (

            <div>
                <div className="landing-container">

                    <section className="landing-header-container">
                        <div className="logo">
                            <img className="icon icons8-Maintenance" width="40" height="40" alt="logo" src="data:image/png;base64," />
                            <h4>Maintenance<span>Minder</span></h4>
                        </div>
                        <a href={process.env.REACT_APP_LOGIN}><button className="start-free-button"> START FREE NOW</button></a>
                    </section>

                    <section className="hero-container">
                        <div className="banner">
                            <div className="banner-logo">
                                <span className="title1">Maintenance<span className="minder-title">Minder</span></span>
                                <span className="title2"><span>YOU LIVE YOUR LIFE.</span>  &nbsp; <u>WE WILL MIND THE MAINTENANCE.</u></span>
                            </div>
                            <a href={process.env.REACT_APP_LOGIN}><button className="register-button" >LOG IN</button></a>
                        </div>
                    </section>
                </div>

                <section className="about-section">
                    <div className="about-header">
                        <h3 className="about-font">MANAGE ALL YOUR ASSETS</h3>

                        <div className="car-container">
                            <div className="slideshow">
                                <div class="slide-wrapper">
                                    <div class="slide"><h1 class="slide-number"></h1></div>
                                    <div class="slide"><h1 class="slide-number"></h1></div>
                                    <div class="slide"><h1 class="slide-number"></h1></div>
                                    <div class="slide"><h1 class="slide-number"></h1></div>
                                    <div class="slide"><h1 class="slide-number"></h1></div>
                                    
                                </div>
                            </div>
                            <div className="about-text">
                                <p>Easy to use smart application to manage</p>
                                <p>all your assets data for maintenance, insurance,</p>
                                <p>and remodeling. Save money and improve value</p>
                                <p>by getting organized. Now is the time to start</p>
                                <p>managing all your assets!</p>
                                <a href={process.env.REACT_APP_LOGIN}><button className="start-free-button2">START FREE NOW</button></a>
                            </div>
                        </div>
                    </div>
                </section>


                <section className="phone-app-section">
                    <div className="phone-app-bgcolor">
                        <h3 className="phone-font">SAVE MONEY &nbsp; <span>IMPROVE VALUE &nbsp;</span> GET ORGANIZED</h3>

                        <div className="phone-container">
                            <div className="phone-text">
                                <p>Improve the value of your assets by avoiding</p>
                                <p>early replacement costs, and keep organized</p>
                                <p>by staying in control of the maintenance of</p>
                                <p>your property.  You live your life, we will</p>
                                <p>mind the maintenance.</p>
                                <a href={process.env.REACT_APP_LOGIN}><button className="start-free-button2">START FREE NOW</button></a>
                            </div>
                            <div className="phone-pic"></div>
                        </div>
                    </div>
                </section>

                <section className="footer">
                    <Footer />
                </section>

            </div>

        );
    }
}